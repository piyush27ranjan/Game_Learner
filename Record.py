...
def OnMouseEvent(event):
	...
	if (event.Message != 512):  # 512 is mouse move
		...
		if (recording == True):
			events.append([event.Position[1], event.Position[0],
                            event.Message, event.Time, "2"])
...
def record():
	...
	for event in events:
		f.write(str(event.pop()) + ',' + str(event.pop()) + ',' + str(event.pop()
                                                                ) + ',' + str(event.pop()) + ',' + str(event.pop()) + '\n')
	f.close()
	print("Ending...")
		...
		for line in fp:
			...
			if (lasttime == False):
				if (eventtype == 0 and eventsubtype == 0):
					...
					print("Source data error! (eventtype=" + str(eventtype) +
					      ", eventsubtype=" + str(eventsubtype) + ")")
					sys.exit(1)
			elif (eventtype == 1):
				# At least 20ms between everything
				time.sleep(max((eventtime - lasttime) / 1000, 0.02))
...
def main():
	...
	parser.add_option("-r", "--record", action="store_true",
	                  dest="do_record", default=False, help="record a session of input")
	parser.add_option("-p", "--play", action="store_true",
	                  dest="do_play", default=False, help="play a session of input")
